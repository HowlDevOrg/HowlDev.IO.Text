...
public class YAMLParser(string file) : ITokenParser {
    ...
    public IEnumerator<(TextToken, string)> GetEnumerator() {
        ...
        if (lines[0].data.StartsWith('-')) {
            foreach ((TextToken, string) item in ReadLinesAsList(lines)) yield return item;
        } else if (lines[0].data.Contains(':')) {
            foreach ((TextToken, string) item in ReadLinesAsDictionary(lines)) yield return item;
        } else {
            string line = "";
        ...
        for (int i = 0; i < lines.Count; i++) {
            ...
            if (string.IsNullOrWhiteSpace(splits[1])) {
                ...
                if (lines[i + 1].data.StartsWith('-')) {
                    foreach ((TextToken, string) item in ReadLinesAsList(NextLineLessOrEqual(lines, i + 1))) yield return item;
                    i++;
                    while (i < lines.Count && lines[i].data.StartsWith('-')) { i++; }

                    i--; // Don't really know what's up with this stuff.
                } else {
                    foreach ((TextToken, string) item in ReadLinesAsDictionary(NextLineLessOrEqual(lines, i + 1))) yield return item;
                    i++;
    ...
    private IEnumerable<(TextToken, string)> ReadLinesAsList(List<(int indentCount, string data)> lines) {
        ...
        for (int i = 0; i < lines.Count; i++) {
            ...
            if (currentIndent < indentCount) {
                foreach ((TextToken, string) item in ReadLinesAsList(NextLineLessOrEqual(lines, i))) yield return item;
                i++;
                while (i < lines.Count && lines[i].indentCount != currentIndent) { i++; }
                ...
                if (lineData.Contains(':')) {
                    foreach ((TextToken, string) item in ReadLinesAsDictionary(NextLineLessOrEqual(lines, i, '-'))) yield return item;
                    i++;
                    while (i < lines.Count && !lines[i].data.StartsWith('-')) { i++; }
